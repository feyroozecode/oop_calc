...
namespace Application{
    class Calculator {
        ...
        private static readonly string split_patern = @"\s*([\+\-\*\/\==\%\V])\s*";
        ...
        public static void mainLoop(){
            ...
            while (parcourir)
            {
                Fun.affiche("Entrer votre operation ou cliquer sur q pour quitter");
                ...
                if(!Regex.IsMatch(userInput, pattern)){
                    Fun.affiche("Veullez entrer des valeur valide");
                } 
                else{
        ...
        public static void launchCalc(String input){
             ...
             // Effecer les sapces separant les nombres des instructions 
                var operations = Regex.Split(input, split_patern );

                // initialisez le resultat, débutant à lelement (index 1)
                
                double dChecker; // pour tester lsi la valeur est double ou pas 
                if(!double.TryParse(operations[0], out dChecker)){
                    Fun.affiche("Vous dévrier entrer des chiffres pour les calcules");
                }
